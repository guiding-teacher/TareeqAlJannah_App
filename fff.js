تمام، أتفهم تماماً أن الحصول على الكود كاملاً وصحيحاً هو الأولوية القصوى لتجنب أي مشاكل متكررة. أعتذر مرة أخرى عن أي تقطيع حدث في الردود السابقة.

سأقدم لك الآن **النسخة الكاملة والمحدثة لكل من `server.js` و `script.js`، بالإضافة إلى أي تعديلات ضرورية في `index.html` و `style.css`**. هذه النسخ يجب أن تحل جميع المشاكل التي ذكرتها وتضيف الميزات الجديدة.

**الرجاء اتباع هذه التعليمات بدقة شديدة:**

1.  **امسح محتويات كل ملف من الملفات الأربعة (`server.js`, `script.js`, `index.html`, `style.css`) بالكامل.**
2.  **الصق الكود الجديد الذي سأقدمه لك (لكل ملف على حدة) في الملف الفارغ الخاص به.**
3.  **احفظ جميع الملفات الأربعة.**
4.  **تأكد من وجود ملف `.env` في مجلد `backend` ويحتوي على:**
    ```
    DB_URI=mongodb://localhost:27017/tareeq_aljannah
    PORT=3000
    ```
5.  **أعد تشغيل الخادم:**
    *   أوقف الخادم القديم (Ctrl+C).
    *   في مجلد `backend`، نفذ: `node server.js`
6.  **افتح المتصفح:** اذهب إلى `http://localhost:3000`.

---

## **1. ملف `server.js` (النسخة الكاملة النهائية)**

**امسح كل محتويات `server.js` ثم الصق هذا الكود كاملاً:**
...
const UserSchema = new mongoose.Schema({
    ...
    settings: {
        ...
        emergencyWhatsapp: { type: String } // جديد: رقم الواتساب للطوارئ
    },
...
io.on('connection', async (socket) => {
    ...
    socket.on('registerUser', async (data) => {
        const { userId, name, photo, emergencyWhatsapp } = data; // استقبال emergencyWhatsapp
...
## **2. ملف `script.js` (النسخة الكاملة النهائية)**

**امسح كل محتويات `script.js` ثم الصق هذا الكود كاملاً:**
...
## **3. ملف `index.html` (النسخة الكاملة النهائية)**

**امسح كل محتويات `index.html` ثم الصق هذا الكود كاملاً:**
...
## **4. ملف `style.css` (النسخة الكاملة النهائية)**

**امسح كل محتويات `style.css` ثم الصق هذا الكود كاملاً:**
